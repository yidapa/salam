...
GXXXX_pick_mols_com_files_dir = './project/'  +  GXXXX  +  '/pick_mols/com-files/opt_jobs/'
...
if (NUM_SERVIERS == 2):
    ...
    retcode = subprocess.call("./bat-g16-opt-freq.sh  "  +  str(COMPUTE_RATIO_SERVIER1)   +  "   %d  "%G_num , shell=True)
    print("run pbs_jobs. retcode = ", retcode)
    
    #------ run remote pbs jobs ---------------------------------------------------
    status, output = subprocess.getstatusoutput('ssh    tucy@node130    "cd   '   +   remote_GXXXX_opt_jobs_dir   +  ' ;    ./bat-g16-opt-freq_node130_133.sh     '  +  str(COMPUTE_RATIO_SERVIER2)   +  '   %d   '%G_num   +  '     > /dev/null   2>&1   &  "  ')  
    print("run remote pbs_jobs status. status, output = ", status, output)

else:
    print("NUM_SERVIERS = ", NUM_SERVIERS)
    retcode = subprocess.call('./bat-g16-opt-freq.sh    1    '    +  '   %d  '%G_num , shell=True)
